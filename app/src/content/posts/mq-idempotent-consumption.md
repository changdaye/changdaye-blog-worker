---
title: '如何保证消息不被重复消费'
subtitle: '从消息幂等性出发理解重复消费与去重策略'
description: '结合 Kafka 等消息队列场景，解释为什么会重复消费，以及如何设计幂等处理。'
date: 2019-11-19
author: "Mr Chang"
tags:
  - MQ
...
有这么个场景。数据 1/2/3 依次进入 kafka，kafka 会给这三条数据每条分配一个 offset，代表这条数据的序号，我们就假设分配的 offset 依次是 152/153/154。消费者从 kafka
去消费的时候，也是按照这个顺序去消费。假如当消费者消费了 offset=153 的这条数据，刚准备去提交 offset 到 zookeeper，此时消费者进程被重启了。那么此时消费过的数据 1/2 的 offset 并没有提交，kafka
